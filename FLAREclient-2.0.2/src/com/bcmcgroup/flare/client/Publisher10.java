...
public class Publisher10 {
	...
	public void publish(String fN, HashMap<String,String> extHdrs, String msg, List<ContentBlock> cBlocks) throws IOException {	
		...
		boolean signResult = Xmldsig.signInboxMessage(taxiiDoc, pks, pksPw, pk, pkPw);
		logger.debug("Publisher10 signInboxMessage status: " + signResult);
		if (signResult) {
			...
			String address = "";
			if ("true".equalsIgnoreCase(config.getProperty("connectingToFLARE"))) {
				address = config.getProperty("taxii10serverUrlInbox") + "/" + URLEncoder.encode(fN, "UTF-8").replace("+", "%20");
			} else {
				address = config.getProperty("taxii10serverUrlInbox");
			}
