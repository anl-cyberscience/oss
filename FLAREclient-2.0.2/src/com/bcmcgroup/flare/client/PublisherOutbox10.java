 ...
 * @version		2.0
 */
public class PublisherOutbox10 {
     ...
     * @throws IOException
     */
    public PublisherOutbox10(Path dir, boolean recursive) throws IOException {
   ...
   public void processEvents() throws IOException {
        for (;;) {
            ...
            for (WatchEvent<?> event: key.pollEvents()) {
                ...
                if (kind.name().equalsIgnoreCase("ENTRY_CREATE")) {
	                ...
	                // publish message
	            	Publisher10 pub = new Publisher10();
     ...
     */
    static void usage() {
    	logger.debug("Usage: java PublisherOutbox10 [-r] dir");
        System.exit(-1);
    }
    	...
    	try {
	        ...
	        if (args.length == 0) {
	            ...
	            if (file.isDirectory() && file.exists()) {
	            	new PublisherOutbox10(Paths.get(pubPath), false).processEvents();
	            } else {
	            ...
	            logger.debug("Publish directory is :  " + file);
	            if (file.isDirectory() && file.exists()) {
	            	new PublisherOutbox10(Paths.get(pubPath), true).processEvents();
	            } else {
	            	logger.debug("Input error, directory:  " + file + " is not valid");
	            }
	        } else {
