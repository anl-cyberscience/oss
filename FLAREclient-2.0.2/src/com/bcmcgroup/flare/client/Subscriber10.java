...
public class Subscriber10 {
	...
	public String poll(String msgId, String fN, String subId, HashMap<String,String> extHdrs, String ebt, String iet, Set<String> cBind, String fileName) {
		try {
			...
			if ("true".equalsIgnoreCase(config.getProperty("connectingToFLARE"))) {
				address = config.getProperty("taxii10serverUrlPoll") + "/" + URLEncoder.encode(fN, "UTF-8").replace("+", "%20");
			} else {
				address = config.getProperty("taxii10serverUrlPoll");
			}
	...
	public void save(Document taxiiDoc, String fileName) {
		...
		try {							
			...
			logger.error("save: IOException on validation:  " + e.getMessage());
			return;
		}
