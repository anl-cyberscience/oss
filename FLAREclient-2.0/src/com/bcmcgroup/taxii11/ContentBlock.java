...
public class ContentBlock {
	...
	public ContentBlock(String cB, File c, String tsL, String padd, Node dsS) throws IOException {
		logger.debug("ContentBlock11 cB: " + cB + " File c: " + c + " tsL: " + tsL + " padd: " + padd + " dsS: " + dsS);	
		content_binding = cB;
		if (c.exists()) {
			logger.debug("ContentBlock11 reading file c.canRead(): " + c.canRead());
			logger.debug("ContentBlock11 reading file c.length(): " + c.length()); 
			logger.debug("ContentBlock11 reading file c: " + c);
			FileInputStream fis = null;
			try {
				fis = new FileInputStream(c);
			    content = readFileToString(fis);
			} catch (IOException e) {
				logger.error("ContentBlock11 IOException e: " + e.getMessage());
				...
				if (fis != null) {
					try {
						...
						logger.error("ContentBlock11 IOException ioe: " + ioe.getMessage());
					}
					fis = null;
				}
			}
			...
			logger.debug("ContentBlock11 content: " + content);
		} else {
